...
from ephios.user_management.forms.events import EventDuplicationForm, EventForm
from ephios.user_management.models import Event, EventType, Shift
...
class EventListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            get_objects_for_user(self.request.user, "user_management.view_event")
            .annotate(
...
class EventDetailView(CustomPermissionRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        if self.request.user.has_perm("user_management.add_event"):
...
class EventUpdateView(CustomPermissionRequiredMixin, UpdateView):
    ...
    permission_required = "user_management.change_event"
...
class EventCreateView(CustomPermissionRequiredMixin, CreateView):
    template_name = "user_management/event_form.html"
    ...
    def get_success_url(self):
        return reverse("user_management:event_createshift", kwargs={"pk": self.object.pk})
...
class EventActivateView(CustomPermissionRequiredMixin, SingleObjectMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(reverse("user_management:event_detail", kwargs={"pk": event.pk}))


class EventDeleteView(CustomPermissionRequiredMixin, DeleteView):
    queryset = Event.all_objects.all()
    permission_required = "user_management.delete_event"
    success_url = reverse_lazy("user_management:event_list")


class EventArchiveView(CustomPermissionRequiredMixin, ListView):
    permission_required = "user_management.view_past_event"
    model = Event
    template_name = "event_management/../templates/user_management/event_archive.html"
...
class EventCopyView(CustomPermissionRequiredMixin, SingleObjectMixin, FormView):
    permission_required = "user_management.add_event"
    model = Event
    template_name = "user_management/event_copy.html"
    ...
    def form_valid(self, form):
        ...
        return redirect(reverse("user_management:event_list"))
