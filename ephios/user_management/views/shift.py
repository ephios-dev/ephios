...
from ephios.user_management.forms.events import ShiftForm
...
class ShiftCreateView(CustomPermissionRequiredMixin, TemplateView):
    ...
    template_name = "user_management/shift_form.html"
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid() and configuration_form.is_valid():
            ...
            if "addAnother" in self.request.POST:
                return redirect(
                    reverse(
                        "user_management:event_createshift", kwargs={"pk": self.kwargs.get("pk")}
                    )
                )
            try:
                self.event.activate()
...
class ShiftUpdateView(CustomPermissionRequiredMixin, SingleObjectMixin, TemplateView):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid() and configuration_form.is_valid():
            ...
            if "addAnother" in self.request.POST:
                return redirect(
                    reverse("user_management:event_createshift", kwargs={"pk": shift.event.pk})
                )
            messages.success(
