...
class GroupForm(ModelForm):
    ...
    def save(self, commit=True):
        ...
        if self.cleaned_data["can_view_past_event"]:
            assign_perm("user_management.view_past_event", group)
        else:
            remove_perm("user_management.view_past_event", group)

        remove_perm("publish_event_for_group", group, Group.objects.all())
        if self.cleaned_data["can_add_event"]:
            assign_perm("user_management.add_event", group)
            assign_perm("user_management.delete_event", group)
            assign_perm(
                "publish_event_for_group", group, self.cleaned_data["publish_event_for_group"]
            )
        else:
            remove_perm("user_management.add_event", group)
            remove_perm("user_management.delete_event", group)
