...
from event_management.utils import CustomDateInput, CustomTimeInput
...
class ShiftForm(ModelForm):
    ...
    def clean(self):
        ...
        if not cleaned_data["meeting_time"] < cleaned_data["start_time"]:
            raise ValidationError("Meeting time must be before start time!")
        return cleaned_data

    def save(self, commit=True):
        shift = super(ShiftForm, self).save(commit)
