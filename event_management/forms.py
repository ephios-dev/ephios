...
class EventForm(ModelForm):
    ...
    def save(self, commit=True):
        no_instance = not self.instance.pk
        event = super().save(commit)
        if "visible_for" in self.changed_data or no_instance:
        ...
        if "responsible_groups" in self.changed_data or no_instance:
            ...
            for group in self.initial["responsible_groups"].difference(
                ...
                if group not in self.cleaned_data["visible_for"]:
                    remove_perm("view_event", group, event)
        if "responsible_persons" in self.changed_data or no_instance:
